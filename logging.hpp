...
static void error(const char* msg, ...) {
  if (LOG_LEVEL & ERROR_LVL) {
    fprintf(stderr, "%s ", ANSI_RED_C "[ERR]" ANSI_COLOR_RESET);
...
static void success(const char* msg, ...) {
  if (LOG_LEVEL & SUCCESS_LVL) {
    printf("%s", ANSI_GRN_C "[SUCESS]" ANSI_COLOR_RESET);
    PRINT_VAR_ARGS(msg, msg);
  }
}
...
static void info(const char* msg, ...) {
  if (LOG_LEVEL & SUCCESS_LVL) {
    printf("%s", ANSI_CYN_C "[INFO]" ANSI_COLOR_RESET);
    PRINT_VAR_ARGS(msg, msg);
  }
}
...
static void debug(const char* msg, ...) {
  if (LOG_LEVEL & DEBUG_LVL) {
    printf("%s", ANSI_MG_C "[DEBUG]" ANSI_COLOR_RESET);
    PRINT_VAR_ARGS(msg, msg);
  }
}
static void warn(const char* msg, ...) {
  if (LOG_LEVEL & WARN_LVL) {
    printf("%s", ANSI_YEL_C "[WARN]" ANSI_COLOR_RESET);
    PRINT_VAR_ARGS(msg, msg);
  }
}
static void critical(const char* msg, ...) {
  if (LOG_LEVEL & CRITICAL_LVL) {
    printf("%s", ANSI_RED_BC "[CRITICAL]" ANSI_COLOR_RESET);
    PRINT_VAR_ARGS(msg, msg);
  }
}
}  // namespace logging
