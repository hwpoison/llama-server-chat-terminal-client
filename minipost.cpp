...
Response httpRequest::post(const std::string url, json payload, const std::function<bool(std::string chunck)>&reader_callback = nullptr) {
    ...
    logging::info("Connecting to %s to port %d", parsed_url.getDomain(), parsed_url.getPort());
    logging::info("Sending:", payload);
    SocketType connection = connect_to(parsed_url.getDomain().c_str(), parsed_url.getPort());
    ...
    while ((bytesRead = recv(connection, buffer, sizeof(buffer), 0)) > 0) {
        ...
        if (reader_callback != nullptr){
            if (!reader_callback(response.body))
                break;
        }
    };
